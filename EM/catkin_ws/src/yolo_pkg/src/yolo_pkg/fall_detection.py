...
# ROS 패키지 내부의 config 모듈에서 설정을 불러옵니다.
...
class EnhancedPoseAnalyzer:
    ...
    def _analyze_vertical_alignment(self, keypoints: np.ndarray) -> float:
        ...
        is_vertical = (
            y_coords["shoulder"] < y_coords["hip"] < y_coords["knee"] < y_coords["ankle"]
        )
        height_diffs = [
            abs(y2 - y1)
            for y1, y2 in zip(
                list(y_coords.values())[:-1], list(y_coords.values())[1:]
            )
        ]
            ...
            for result in results:
                ...
                for person_idx, kps in enumerate(keypoints):
                    ...
                    processed_frame = self.visualize_detection(
                        processed_frame, kps, is_fallen, person_debug_info
                    )
...
# 독립 테스트용 main 함수 (실제 ROS 노드에서는 yolo_main.py에서 사용)
async def main():
    config = Config()

    try:
        # 비디오 캡처 초기화 (웹캠 사용 또는 비디오 파일 경로 지정)
        video_path = 0  # 웹캠 사용
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            raise IOError("비디오 소스를 열 수 없습니다.")
        ...
        while True:
            ...
            # 화면 크기 조정 (config에 정의된 display_size 사용)
